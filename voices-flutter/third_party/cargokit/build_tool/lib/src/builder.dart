...
class RustBuilder {
  ...
  Future<String> build() async {
    ...
    // iOS pod integration only needs static library artifacts.
    // Forcing staticlib avoids unnecessary cdylib link failures on Apple toolchains.
    final forceStaticLibOnly =
        !environment.isAndroid && target.darwinPlatform != null;

    final cargoSubcommand = forceStaticLibOnly ? 'rustc' : 'build';
    final cargoArgs = <String>[
      cargoSubcommand,
      if (forceStaticLibOnly) '--crate-type=staticlib',
      ...extraArgs,
      '--manifest-path',
      manifestPath,
      '-p',
